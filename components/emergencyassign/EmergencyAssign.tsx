...
import { Search, MapPin, ArrowLeft, Calendar, Clock } from "lucide-react";
import { axiosInstance } from "@/lib/axiosInstance";
import { toast } from "react-toastify";
import { decryptId } from "@/lib/crypto";
import moment from "moment";

export function EmergencyAssign() {
  ...
  const [docEmergencyDate, setDocEmergencyDate] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchDocDetails = async () => {
      try {
        ...
        const res = await axiosInstance.get(`/emergencydocs/${decryptedId}`);
        const doc = res.data;

        setDocTitle(doc.title || "");
        setDocDesc(doc.description || "");
        setDocEmergencyDate(doc.emergencyDate || "");
        setDocStartTime(doc.startTime || "");
        ...
        const mappedVillageIds =
          doc.emergencyAddresses?.map((addr: any) => addr.village?.id).filter(Boolean) || [];
        ...
        const mappedVillages =
          doc.emergencyAddresses?.map((addr: any) => addr.village).filter(Boolean) || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosInstance.put(`/emergencydocs/updateaddress/${decryptedId}`, {
        provinceId: provinceId ? Number(provinceId) : null,
        districtId: districtId ? Number(districtId) : null,
        villageId: selectedVillages,
      });

      toast.success("ກຳນົດບ້ານແຈ້ງການມອດໄຟສຸກເສີນສຳເລັດ");
      router.push("/emergency");
    } catch (err: any) {
      console.error("Failed to update emergency villages:", err);
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-4 border-t border-theme">
                    ...
                    <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-100/80 dark:border-slate-800/80">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 leading-tight">
                          {docEmergencyDate ? moment(docEmergencyDate).format("DD/MM/YYYY") : "-"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-100/80 dark:border-slate-800/80">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-400 font-semibold leading-none mb-1">ເວລາ</span>
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 leading-tight">
                          {docStartTime && docEndTime ? `${docStartTime} - ${docEndTime}` : "-"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {districtId && (
                    <div className="md:col-span-2 flex flex-col gap-2">
                      <label className="text-xs font-semibold uppercase tracking-wide text-theme-secondary">
                        ບ້ານທີ່ໄດ້ຮັບຜົນກະທົບ * {selectedVillages.length > 0 && `(ເລືອກແລ້ວ ${selectedVillages.length} ບ້ານ)`}
                      </label>
