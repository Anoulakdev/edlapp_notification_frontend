...
export function CutpowerReportManagement() {
  // Filter States
  ...
  const fetchReportData = async (
    ...
    try {
      ...
      console.error("Failed to fetch cutpower report:", err);
  ...
  const handleExportExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Cutpower Outage Report");
    ...
    XLSX.writeFile(workbook, `cutpower_report_${startDate}_to_${endDate}.xlsx`);
  ...
  const handleExportPDF = async () => {
    ...
    let allData: CutpowerDocReportItem[] = [];
    try {
      const params: Record<string, any> = {
        page: 1,
        limit: 9999,
    ...
    allData.forEach((d) => {
      if (d.cutpowerAddresses) {
        ...
        d.cutpowerAddresses.forEach((a) => { sumUsers += Number(a.userCount) || 0; });
      }
    });
    ...
    setLao(15);
    doc.text("ລາຍງານແຈ້ງການຕັດໄຟ", marginL, 20);
    ...
    const statCards = [
      ...
      {
        label: "ບ້ານທີ່ແຈ້ງການຕັດໄຟທັງໝົດ",
    ...
    const tableRows = allData.map((d, index) => {
      ...
      const rowUsers =
        d.cutpowerAddresses?.reduce((acc, a) => acc + (Number(a.userCount) || 0), 0) || 0;

      return [
        String(index + 1),
        d.title || "",
        d.description || "",
        d.cutpowerDate ? moment(d.cutpowerDate).format("DD/MM/YYYY") : "-",
    ...
    // ── 7. Render autoTable ─────────────────────────────────────────────────
    const _baseW = [10, 45, 45, 30, 28, 28, 75, 26]; // 8 cols, sum = 287
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        drawMixed("ລາຍງານແຈ້ງການຕັດໄຟ", marginL, pageH - 6, 6.5);
    ...
    // ── 8. Save file ────────────────────────────────────────────────────────
    doc.save(`cutpower_report_${startDate}_to_${endDate}.pdf`);
  ...
  const stats = useMemo(() => {
    ...
    reportData.forEach((doc) => {
      if (doc.cutpowerAddresses && Array.isArray(doc.cutpowerAddresses)) {
        totalVillages += doc.cutpowerAddresses.length;
        doc.cutpowerAddresses.forEach((addr) => {
    ...
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-6 font-sans text-slate-800 dark:text-slate-100 space-y-6 print:bg-white print:p-0">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">
                ຕົວກອງຂໍ້ມູນລາຍງານແຈ້ງການຕັດໄຟ
              </h2>
      ...
      {!hasSearched && (
        <div className="relative overflow-hidden bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/30 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950/20 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-10 md:p-14 text-center select-none shadow-xl shadow-slate-200/50 dark:shadow-none space-y-6 transition-all">
          ...
          <div className="relative z-10 w-24 h-24 bg-gradient-to-br from-amber-500 via-orange-600 to-amber-700 text-white rounded-3xl flex items-center justify-center mx-auto shadow-xl shadow-amber-500/25 border border-white/20">
            <Zap className="w-12 h-12 text-white drop-shadow-md animate-pulse" />
          ...
          <div className="relative z-10 space-y-2 max-w-lg mx-auto">
            <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
              ກະລຸນາເລືອກຕົວກອງເພື່ອດຶງຂໍ້ມູນລາຍງານແຈ້ງການຕັດໄຟ
            </h3>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/30 dark:shadow-none overflow-hidden">
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs px-3 py-1 bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 rounded-full font-bold">
                  {total} ລາຍການ
                </span>
              </div>
                  ...
                  {loading ? (
                    ...
                    reportData.map((item, idx) => {
                      ...
                      return (
                        ...
                        >
                          {/* # */}
                          <td className="py-4 px-4 text-center font-bold text-slate-400 group-hover:text-amber-600">
                            {rowNum}
                          </td>
                          ...
                          <td className="py-4 px-4">
                            {item.cutpowerAddresses && item.cutpowerAddresses.length > 0 ? (
                              <div className="flex flex-wrap gap-1 max-w-[280px]">
                                {item.cutpowerAddresses.slice(0, 3).map((addr) => (
                                  <span
                                    key={addr.id}
                                ...
                                {item.cutpowerAddresses.length > 3 && (
                                  <span className="text-[10px] px-2 py-0.5 bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 rounded-md font-bold">
                                    +{item.cutpowerAddresses.length - 3} ບ້ານ
                                  </span>
                                )}
                              </div>
                            ) : (
