...
export interface TurnoffAddress {
  id?: number;
  villageId: number;
...
export interface TurnoffDocReportItem {
  id: number;
  title: string;
...
interface TurnoffReportPDFProps {
  data: TurnoffDocReportItem[];
  startDate: string;
  endDate: string;
  provinceName?: string;
  districtName?: string;
...
export const TurnoffReportPDF: React.FC<TurnoffReportPDFProps> = ({
  ...
  return (
    <Document title="ລາຍງານແຈ້ງການມອດໄຟ">
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            <Text style={styles.reportTitle}>ລາຍງານແຈ້ງການມອດໄຟ</Text>
          </View>
        ...
        <View style={styles.statsContainer}>
          ...
          <View style={[styles.statCard, { borderLeftColor: "#7c3aed" }]}>
            <Text style={styles.statLabel}>ບ້ານທີ່ຈະມອດໄຟທັງໝົດ</Text>
        ...
        <View style={styles.table}>
          ...
          {data.map((item, index) => {
            ...
            const rowUsers =
              item.turnoffAddresses?.reduce((acc, a) => acc + (Number(a.userCount) || 0), 0) || 0;

            const fmtStart = item.startDate ? moment(item.startDate).format("DD/MM/YYYY") : "";
            const fmtEnd = item.endDate ? moment(item.endDate).format("DD/MM/YYYY") : "";
            const dateRange =
              fmtStart && fmtEnd && fmtStart !== fmtEnd
                ? `${fmtStart} – ${fmtEnd}`
                : fmtStart || fmtEnd || "-";
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>ລາຍງານແຈ້ງການມອດໄຟ</Text>
          <Text
