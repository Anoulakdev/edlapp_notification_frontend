...
export interface RegisterMeterReportItem {
  id: number;
  fullName: string;
  phone: string;
  accountNear: string;
...
interface RegistermeterReportPDFProps {
  data: RegisterMeterReportItem[];
  startDate: string;
  endDate: string;
  provinceName?: string;
  districtName?: string;
...
export const RegistermeterReportPDF: React.FC<RegistermeterReportPDFProps> = ({
  data,
  startDate,
  endDate,
  ...
  return (
    <Document title="ລາຍງານການຂໍໝໍ້ນັບໄຟໃໝ່">
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            <Text style={styles.reportTitle}>ລາຍງານການຂໍໝໍ້ນັບໄຟໃໝ່</Text>
          </View>
        ...
        <View style={styles.statsContainer}>
          <View style={[styles.statCard, { borderLeftColor: "#2563eb" }]}>
            <Text style={styles.statLabel}>ເອກະສານຂໍໝໍ້ນັບໄຟທັງໝົດ</Text>
            <Text style={[styles.statValue, { color: "#2563eb" }]}>
              {data.length.toLocaleString()} ລາຍການ
            </Text>
          </View>
          <View style={[styles.statCard, { borderLeftColor: "#7c3aed" }]}>
            <Text style={styles.statLabel}>ບ້ານທີ່ມີການຂໍໝໍ້ນັບໄຟ</Text>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableHeaderRow} fixed>
            ...
            <Text style={[styles.tableHeaderCell, { width: "16%" }]}>ຊື່ ແລະ ນາມສະກຸນ</Text>
            <Text style={[styles.tableHeaderCell, { width: "10%" }]}>ເບີໂທ</Text>
            <Text style={[styles.tableHeaderCell, { width: "11%" }]}>ບັນຊີໄກ້ກຽງ</Text>
            <Text style={[styles.tableHeaderCell, { width: "11%" }]}>ສະຖານະ</Text>
            <Text style={[styles.tableHeaderCell, { width: "9%" }]}>ຊ່ອງທາງ</Text>
            <Text style={[styles.tableHeaderCell, { width: "10%" }]}>ແຂວງ</Text>
            <Text style={[styles.tableHeaderCell, { width: "10%" }]}>ເມືອງ</Text>
            <Text style={[styles.tableHeaderCell, { width: "11%" }]}>ບ້ານ</Text>
          </View>
          ...
          {data.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <Text style={[styles.tableCell, { width: "16%" }]}>{item.fullName || "-"}</Text>
                <Text style={[styles.tableCell, { width: "10%" }]}>{item.phone || "-"}</Text>
                <Text style={[styles.tableCell, { width: "11%" }]}>{item.accountNear || "-"}</Text>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>ລາຍງານການຂໍໝໍ້ນັບໄຟໃໝ່</Text>
          <Text
