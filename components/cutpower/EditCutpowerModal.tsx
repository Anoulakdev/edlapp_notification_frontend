...
export function EditCutpowerModal({ open, onClose, selectedDoc, onRefresh }: EditCutpowerModalProps) {
  ...
  const handleEdit = async () => {
    ...
    const result = editCutpowerSchema.safeParse({
      title,
      description,
      cutpowerDate,
      file,
    ...
    try {
      ...
      await axiosInstance.put(`/cutpowerdocs/${selectedDoc.id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      toast.success("ແກ້ໄຂເອກະສານແຈ້ງການຕັດໄຟສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
      console.error("Failed to edit cutpower document:", err);
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂເອກະສານແຈ້ງການຕັດໄຟ" size="xl">
      {loadingDoc ? (
        ...
        <div className="space-y-4" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
          ...
          <div className="flex gap-3 pt-3 border-t border-theme">
            ...
            <Button variant="primary" onClick={handleEdit} loading={saving} disabled={loadingDoc} className="flex-1">
              ບັນທຶກ
            </Button>
          </div>
        </div>
