...
export function ViewCutpowerModal({ open, onClose, selectedDoc }: ViewCutpowerModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`ເບິ່ງເອກະສານ: ແຈ້ງການຕັດໄຟ`} size="2xl">
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-xs text-slate-400 font-medium">ວັນທີຕັດໄຟ</span>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-350">
                  {selectedDoc ? moment(selectedDoc.cutpowerDate).format("DD/MM/YYYY") : "-"}
                </span>
              </div>
            </div>
          </div>
