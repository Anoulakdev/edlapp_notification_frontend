...
export function TurnoffAssign() {
  ...
  useEffect(() => {
    ...
    const fetchDocDetails = async () => {
      try {
        ...
        const res = await axiosInstance.get(`/turnoffdocs/${decryptedId}`);
        const doc = res.data;
        ...
        const mappedVillageIds =
          doc.turnoffAddresses?.map((addr: any) => addr.village?.id).filter(Boolean) || [];
        ...
        const mappedVillages =
          doc.turnoffAddresses?.map((addr: any) => addr.village).filter(Boolean) || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosInstance.put(`/turnoffdocs/updateaddress/${decryptedId}`, {
        provinceId: provinceId ? Number(provinceId) : null,
        districtId: districtId ? Number(districtId) : null,
        villageId: selectedVillages,
      });

      toast.success("ກຳນົດບ້ານທີ່ຈະມອດໄຟສຳເລັດ");
      router.push("/turnoff");
    } catch (err: any) {
      console.error("Failed to update outage villages:", err);
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      {/* <span className="text-xs text-slate-400 font-medium">ຫົວຂໍ້ເອກະສານ:</span> */}
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 leading-tight">
                          {docStartDate && docEndDate
                            ? moment(docStartDate).format("DD/MM/YYYY") === moment(docEndDate).format("DD/MM/YYYY")
                              ? moment(docStartDate).format("DD/MM/YYYY")
                              : `${moment(docStartDate).format("DD/MM/YYYY")} - ${moment(docEndDate).format("DD/MM/YYYY")}`
                            : "-"}
                        </span>
                      </div>
                  ...
                  {districtId && (
                    <div className="md:col-span-2 flex flex-col gap-2">
                      <label className="text-xs font-semibold uppercase tracking-wide text-theme-secondary">
                        ບ້ານທີ່ຈະມອດໄຟ * {selectedVillages.length > 0 && `(ເລືອກແລ້ວ ${selectedVillages.length} ບ້ານ)`}
                      </label>
