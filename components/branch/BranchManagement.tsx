...
export function BranchManagement() {
  ...
  const [docs, setDocs] = useState<Branch[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<Branch | null>(null);
  ...
  const openEdit = (doc: Branch) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: Branch) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/branchs/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນສາຂາແຂວງສຳເລັດ");
    } catch (err) {
  ...
  const columns = useMemo<ColumnDef<Branch>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          const hasRepairDistricts = !!(doc.repairDistricts && doc.repairDistricts.length > 0);
          return (
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <ButtonTooltip text={hasRepairDistricts ? "ບໍ່ສາມາດລົບໄດ້" : "ລົບ"}>
                <button
                  onClick={() => openDelete(doc)}
                  disabled={hasRepairDistricts}
                  className={`p-2 rounded-xl transition-colors shrink-0 ${hasRepairDistricts
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
                  ...
                  {headerGroup.headers.map((header) => {
                    const widthClass =
                      header.id === "index" ? "w-[10%] min-w-[70px]" :
                        header.id === "name" ? "w-[35%] min-w-[180px]" :
                          header.id === "code" ? "w-[15%] min-w-[100px]" :
                            header.id === "creator" ? "w-[20%] min-w-[140px]" :
                              header.id === "createdAt" ? "w-[12%] min-w-[120px]" :
                                header.id === "actions" ? "w-[8%] min-w-[80px]" : "";
                    return (
                      <th
                        key={header.id}
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນສາຂາແຂວງ</p>
                      </div>
                    </div>
                  </td>
                </tr>
                    ...
                    {row.getVisibleCells().map((cell) => {
                      const widthClass =
                        cell.column.id === "index" ? "w-[10%] min-w-[70px]" :
                          cell.column.id === "name" ? "w-[35%] min-w-[180px]" :
                            cell.column.id === "code" ? "w-[15%] min-w-[100px]" :
                              cell.column.id === "creator" ? "w-[20%] min-w-[140px]" :
                                cell.column.id === "createdAt" ? "w-[12%] min-w-[120px]" :
                                  cell.column.id === "actions" ? "w-[8%] min-w-[80px]" : "";
                      return (
      ...
      {/* MODALS */}
      <AddBranchModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />
