...
import { editBranchSchema, Branch } from "@/schemas/branch";
import { toast } from "react-toastify";

interface EditBranchModalProps {
  open: boolean;
  onClose: () => void;
  selectedDoc: Branch | null;
  onRefresh: () => void;
}

export function EditBranchModal({ open, onClose, selectedDoc, onRefresh }: EditBranchModalProps) {
  ...
  useEffect(() => {
    if (open && selectedDoc) {
      const fetchDocDetails = async () => {
        try {
          setLoadingDoc(true);
          const res = await axiosInstance.get(`/branchs/${selectedDoc.id}`);
          const doc = res.data;
          ...
          setErrors({ apiError: "ບໍ່ສາມາດໂຫຼດຂໍ້ມູນສາຂາແຂວງໄດ້" });
        } finally {
          setLoadingDoc(false);
        }
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axiosInstance.put(`/branchs/${selectedDoc.id}`, {
        name,
        code: code ? code.trim() : undefined,
      });

      toast.success("ແກ້ໄຂຂໍ້ມູນສາຂາແຂວງສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂຂໍ້ມູນສາຂາແຂວງ" size="md">
      {loadingDoc ? (
