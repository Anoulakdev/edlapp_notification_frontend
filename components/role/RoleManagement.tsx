...
export function RoleManagement() {
  ...
  const [docs, setDocs] = useState<Role[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<Role | null>(null);
  ...
  const fetchDocs = useCallback(async () => {
    try {
      ...
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນສິດຜູ້ໃຊ້ງານໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (doc: Role) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: Role) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/roles/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນສິດຜູ້ໃຊ້ງານສຳເລັດ");
    } catch (err) {
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
      ...
      >
        ...
        >
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              type="text"
              placeholder="ຄົ້ນຫາສິດຜູ້ໃຊ້ງານ..."
              value={search}
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນສິດຜູ້ໃຊ້ງານ</p>
                      </div>
                    </div>
                  </td>
                </tr>
      ...
      {/* MODALS */}
      <AddRoleModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />
