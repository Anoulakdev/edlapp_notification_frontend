...
// Modals
import { AddRegistermeterModal } from "./AddRegistermeterModal";
import { EditRegistermeterModal } from "./EditRegistermeterModal";
...
export function RegistermeterManagement() {
  const router = useRouter();
  const [docs, setDocs] = useState<RegisterMeter[]>([]);
  ...
  // Load register meter documents from backend
  const fetchDocs = useCallback(async (
    ...
    try {
      ...
      const params: Record<string, any> = {
        ...
        meterStatusId: statusId || undefined,
        sourcetypeId: sTypeId || undefined,
      };

      if (isCustomer) {
        params.userAppId = currentUserId || undefined;
      } else {
        params.filterMyDocs = myDocsOnly || undefined;
      }
      ...
      console.error("Failed to load register meter documents:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນຂໍໝໍ້ນັບໄຟໄດ້");
    } finally {
      setLoading(false);
    }
  }, [currentUserRoleId, currentUserId]);
  ...
  const openEdit = (doc: RegisterMeter) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: RegisterMeter) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };

  const openForward = (doc: RegisterMeter, mode: "create" | "update") => {
    setSelectedDoc(doc);
    setForwardMode(mode);
    setForwardOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/registermeters/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນຂໍໝໍ້ນັບໄຟສຳເລັດ");
    } catch (err) {
      console.error("Failed to delete register meter:", err);
      toast.error("ເກີດຂໍ້ຜິດພາດໃນການລົບຂໍ້ມູນ");
    }
  };

  // Columns definition
  const columns = useMemo<ColumnDef<RegisterMeter>[]>(
    () => [
      {
        accessorKey: "id",
        header: "ເລກທີ",
      ...
      },
      {
        ...
        header: "ວັນທີແຈ້ງ",
        cell: ({ row }) => (
          <div className="flex flex-col text-xs text-slate-650 dark:text-slate-350">
      ...
      },
      {
        ...
        header: "ຊື່ຜູ້ຂໍໝໍ້ນັບໄຟ",
        cell: ({ row }) => (
          <div className="text-sm font-semibold whitespace-nowrap" style={{ color: "rgb(var(--text-primary))" }}>
      ...
      },
      {
        ...
        header: "ເບີໂທ",
        cell: ({ row }) => (
          <span className="text-xs font-medium text-slate-650 dark:text-slate-350 font-mono">
            {row.original.phone}
          </span>
        ),
      },
      {
        accessorKey: "accountNear",
        header: "ເລກບັນຊີໃກ້ຄຽງ",
        cell: ({ row }) => (
          <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 font-mono">
            {row.original.accountNear}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs whitespace-nowrap font-semibold ${badgeClass}`}>
              {doc.sourcetype?.name || "-"}
            </span>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          let badgeClass = "bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-300";
          if (doc.meterStatusId === 1) {
            badgeClass = "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300";
          } else if (doc.meterStatusId === 2) {
            badgeClass = "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300";
          } else if (doc.meterStatusId === 3) {
            badgeClass = "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300";
          } else if (doc.meterStatusId === 4) {
            badgeClass = "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300";
          }
          return (
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs whitespace-nowrap font-semibold ${badgeClass}`}>
              {doc.meterStatus?.callcenter || `ສະຖານະ ${doc.meterStatusId}`}
            </span>
          );
        },
      },
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
        ...
        {!isCustomer && (
          <div className="sm:ml-auto flex items-center">
            <button
              onClick={openAdd}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-brand hover:opacity-90 transition-all duration-300 shadow-sm shadow-blue-500/10 cursor-pointer"
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2 sm:ml-auto">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm transition-all cursor-pointer"
              style={{
                fontFamily: "inherit",
        ...
        {showFilters && (
          <div
            className="flex flex-col sm:flex-row flex-wrap gap-4 px-4 sm:px-6 py-4 border-b text-sm items-end animate-in fade-in slide-in-from-top-2"
          ...
          >
              ...
              <div className="w-full sm:w-48 flex flex-col gap-1.5">
                ...
                <div className="relative w-full">
                  <select
                    ...
                    className="w-full pl-3 pr-8 py-2 rounded-xl border outline-none text-xs appearance-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{ fontFamily: "'Noto Sans Lao', sans-serif", background: "rgb(var(--card))", color: "rgb(var(--text-primary))", borderColor: "rgb(var(--border))" }}
                  >
                    <option value="">ທັງໝົດ</option>
                    {districts.map((d) => (
            ...
            <div className="w-full sm:w-48 flex flex-col gap-1.5">
              ...
              <div className="relative w-full">
                ...
                >
                  <option value="">ທັງໝົດ</option>
                  {meterStatuses.map((s) => (
                    <option key={s.id} value={s.id}>{s.callcenter || `ສະຖານະ ${s.id}`}</option>
                  ))}
                </select>
                <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-slate-400">
                  <ChevronDown className="w-3.5 h-3.5" />
            ...
            {/* My Docs Toggle */}
            {!isCustomer && (
              <div className="flex items-center pb-2.5 select-none">
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນຂໍໝໍ້ນັບໄຟ</p>
                      </div>
                    </div>
                  </td>
                </tr>
      ...
      <ViewRegistermeterModal
        open={viewOpen}
        onClose={() => setViewOpen(false)}
        selectedDoc={selectedDoc}
      />

      <ForwardRegistermeterModal
        open={forwardOpen}
