...
import { createRegistermeterSchema } from "@/schemas/registermeter";
import { toast } from "react-toastify";
import { loadLeaflet } from "@/lib/utils";

interface AddRegistermeterModalProps {
  open: boolean;
  onClose: () => void;
  onRefresh: () => void;
}

export function AddRegistermeterModal({ open, onClose, onRefresh }: AddRegistermeterModalProps) {
    ...
    if (open) {
      const fetchInitialData = async () => {
        try {
          ...
          console.error("Failed to load initial data for register meter:", err);
        }
      };
      fetchInitialData();
    }
  ...
  const handleAdd = async () => {
    const result = createRegistermeterSchema.safeParse({
      fullName,
      phone,
      accountNear,
    ...
    try {
      ...
      await axiosInstance.post("/registermeters", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      toast.success("ເພີ່ມຂໍ້ມູນຂໍໝໍ້ນັບໄຟສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
      console.error("Failed to add register meter:", err);
  ...
  return (
    <Modal open={open} onClose={onClose} title="ເພີ່ມຂໍ້ມູນຂໍໝໍ້ນັບໄຟໃໝ່" size="2xl">
