...
export function EmergencyManagement() {
  ...
  const [docs, setDocs] = useState<EmergencyDoc[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<EmergencyDoc | null>(null);
  ...
  const fetchDocs = useCallback(async (searchVal = "", date = "", provId = "", distId = "", myDocsOnly = false) => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/emergencydocs", {
        params: {
          search: searchVal.trim() || undefined,
          emergencyDate: date || undefined,
      ...
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນແຈ້ງການມອດໄຟສຸກເສີນໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchDocs(debouncedSearch, emergencyDate, effectiveProvinceId, selectedDistrictId, filterMyDocs);
  }, [debouncedSearch, emergencyDate, effectiveProvinceId, selectedDistrictId, filterMyDocs, fetchDocs]);

  // Keep track of latest filter values in a ref to avoid reconnecting SSE on every keystroke/filter change
  const filterRef = useRef({ debouncedSearch, emergencyDate, effectiveProvinceId, selectedDistrictId, filterMyDocs });
  useEffect(() => {
  ...
  const openEdit = (doc: EmergencyDoc) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openAssignVillages = (doc: EmergencyDoc) => {
    const encrypted = encryptId(doc.id);
    router.push(`/emergencyassign?id=${encrypted}`);
  };

  const openDelete = (doc: EmergencyDoc) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/emergencydocs/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນແຈ້ງການມອດໄຟສຸກເສີນສຳເລັດ");
  ...
  // Columns definition
  const columns = useMemo<ColumnDef<EmergencyDoc>[]>(
    () => [
      {
        accessorKey: "id",
        header: "ເລກທີ",
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="text-xs font-medium text-slate-600 dark:text-slate-350 flex items-center gap-1.5 whitespace-nowrap">
              ...
              {moment(doc.emergencyDate).format("DD/MM/YYYY")}
            </span>
          );
        },
      },
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-semibold bg-brand/10 text-brand dark:bg-brand/20 whitespace-nowrap">
              ...
              {doc.startTime && doc.endTime ? `${doc.startTime} - ${doc.endTime}` : "-"}
            </span>
          );
        },
      },
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
      ...
      {/* MODALS */}
      <AddEmergencyModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={() => fetchDocs(search, emergencyDate, effectiveProvinceId, selectedDistrictId, filterMyDocs)} />

      <EditEmergencyModal
        open={editOpen}
        onClose={() => setEditOpen(false)}
        selectedDoc={selectedDoc}
        onRefresh={() => fetchDocs(search, emergencyDate, effectiveProvinceId, selectedDistrictId, filterMyDocs)}
      />
