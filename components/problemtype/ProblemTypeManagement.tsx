...
import { ProblemType } from "@/schemas/problemtype";

// Modals
import { AddProblemTypeModal } from "./AddProblemTypeModal";
import { EditProblemTypeModal } from "./EditProblemTypeModal";
import { DeleteProblemTypeModal } from "./DeleteProblemTypeModal";

const ROWS_PER_PAGE = 10;

export function ProblemTypeManagement() {
  const [docs, setDocs] = useState<ProblemType[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<ProblemType | null>(null);

  // Fetch Problem Types from backend
  const fetchDocs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/problemtypes");
      const data = Array.isArray(res.data) ? res.data : (res.data.data || []);
      setDocs(data);
    } catch (err) {
      console.error("Failed to load problem types:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນປະເພດບັນຫາໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (doc: ProblemType) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: ProblemType) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/problemtypes/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນປະເພດບັນຫາສຳເລັດ");
    } catch (err) {
      console.error("Failed to delete problem type:", err);
      toast.error("ເກີດຂໍ້ຜິດພາດໃນການລົບຂໍ້ມູນ");
    }
  };

  // Columns definition
  const columns = useMemo<ColumnDef<ProblemType>[]>(
    () => [
      {
        id: "index",
        header: "ລຳດັບ",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          return (
            <span className="text-sm text-slate-600 dark:text-slate-350 font-medium">
              {doc.code || "-"}
            </span>
          );
        },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          const hasProblemDocs = !!(doc.problemdocs && doc.problemdocs.length > 0);
          return (
            <div className="flex items-center gap-1.5 shrink-0">
              <ButtonTooltip text="ແก້ໄຂ">
                <button
              ...
              <ButtonTooltip text={hasProblemDocs ? "ບໍ່ສາມາດລົບໄດ້" : "ລົບ"}>
                <button
  ...
  const filteredDocs = useMemo(() => {
    ...
    return docs.filter(
      (doc) =>
        ...
        (doc.code && doc.code.toLowerCase().includes(s))
    );
  }, [docs, search]);
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນປະເພດບັນຫາ</p>
                      </div>
                    </div>
                  </td>
                </tr>
      ...
      {/* MODALS */}
      <AddProblemTypeModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />

      <EditProblemTypeModal
        open={editOpen}
