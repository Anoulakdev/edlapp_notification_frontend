...
import { updateRepairDistrictSchema, RepairDistrict } from "@/schemas/repairdistrict";
import { toast } from "react-toastify";

interface EditRepairDistrictModalProps {
  open: boolean;
  doc: RepairDistrict | null;
  onClose: () => void;
  onRefresh: () => void;
}

export function EditRepairDistrictModal({
  open,
  doc,
  onClose,
  onRefresh,
}: EditRepairDistrictModalProps) {
  ...
  useEffect(() => {
    if (open && doc) {
      setBranchId(doc.branchId ? String(doc.branchId) : "");
      setName(doc.name || "");
      setCode(doc.code || "");
      setErrors({});
  ...
  const handleSubmit = async () => {
    ...
    const result = updateRepairDistrictSchema.safeParse({
    ...
    try {
      await axiosInstance.put(`/repairdistricts/${doc.id}`, {
        branchId: Number(branchId),
        name: name.trim(),
        code: code ? code.trim() : undefined,
      });

      toast.success("ແກ້ໄຂຂໍ້ມູນສູນສ້ອມແປງເມືອງສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
      console.error("Failed to edit repair district:", err);
      const errMsg =
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂຂໍ້ມູນສູນສ້ອມແປງເມືອງ" size="md">
      <div
        className="space-y-4"
