...
export function EditTopicModal({ open, onClose, selectedDoc, onRefresh }: EditTopicModalProps) {
  ...
  useEffect(() => {
    if (open && selectedDoc) {
      const fetchDocDetails = async () => {
        try {
          setLoadingDoc(true);
          const res = await axiosInstance.get(`/topics/${selectedDoc.id}`);
          const doc = res.data;
          ...
          setErrors({ apiError: "ບໍ່ສາມາດໂຫຼດຂໍ້ມູນຫົວຂໍ້ໄດ້" });
        } finally {
          setLoadingDoc(false);
        }
      };
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂຫົວຂໍ້ການສົນທະນາ" size="md">
      {loadingDoc ? (
