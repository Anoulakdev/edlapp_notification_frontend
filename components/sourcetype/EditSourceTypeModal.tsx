...
import { editSourceTypeSchema } from "@/schemas/sourcetype";
import { toast } from "react-toastify";

interface EditSourceTypeModalProps {
  open: boolean;
  onClose: () => void;
...
export function EditSourceTypeModal({ open, onClose, selectedDoc, onRefresh }: EditSourceTypeModalProps) {
  ...
  useEffect(() => {
    if (open && selectedDoc) {
      const fetchDocDetails = async () => {
        try {
          setLoadingDoc(true);
          const res = await axiosInstance.get(`/sourcetypes/${selectedDoc.id}`);
          const doc = res.data;
          ...
          console.error("Failed to load source type details:", err);
          setErrors({ apiError: "ບໍ່ສາມາດໂຫຼດຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງໄດ້" });
        } finally {
          setLoadingDoc(false);
        }
      };
  ...
  const handleSubmit = async () => {
    ...
    const result = editSourceTypeSchema.safeParse({
      name,
    ...
    try {
      await axiosInstance.put(`/sourcetypes/${selectedDoc.id}`, {
        name,
        description: description || null,
      });

      toast.success("ແກ້ໄຂຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງ" size="md">
      {loadingDoc ? (
        ...
        <div className="space-y-4" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
          ...
          <div className="space-y-4">
            <Input
              label="ຊື່ຊ່ອງທາງຮັບແຈ້ງ *"
              placeholder="ປ້ອນຊື່ຊ່ອງທາງຮັບແຈ້ງ..."
              value={name}
            ...
            <Textarea
              label="ຄຳອະທິບາຍ"
              placeholder="ປ້ອນຄຳອະທິບາຍຊ່ອງທາງຮັບແຈ້ງ..."
              value={description}
