...
import { createSourceTypeSchema } from "@/schemas/sourcetype";
...
export function AddSourceTypeModal({ open, onClose, onRefresh }: AddSourceTypeModalProps) {
  ...
  const handleSubmit = async () => {
    const result = createSourceTypeSchema.safeParse({
      name,
    ...
    try {
      await axiosInstance.post("/sourcetypes", {
        name,
        description: description || null,
      });

      toast.success("ເພີ່ມຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
      console.error("Failed to add source type:", err);
  ...
  return (
    <Modal open={open} onClose={onClose} title="ເພີ່ມຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງ" size="md">
