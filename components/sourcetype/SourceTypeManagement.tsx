...
import { SourceType } from "@/schemas/sourcetype";

// Modals
import { AddSourceTypeModal } from "./AddSourceTypeModal";
import { EditSourceTypeModal } from "./EditSourceTypeModal";
import { DeleteSourceTypeModal } from "./DeleteSourceTypeModal";

const ROWS_PER_PAGE = 10;

export function SourceTypeManagement() {
  const router = useRouter();
  const [docs, setDocs] = useState<SourceType[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<SourceType | null>(null);
  ...
  const fetchDocs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/sourcetypes");
      const data = Array.isArray(res.data) ? res.data : (res.data.data || []);
      setDocs(data);
    } catch (err) {
      console.error("Failed to load source types:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (doc: SourceType) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: SourceType) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/sourcetypes/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງສຳເລັດ");
    } catch (err) {
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
      ...
      >
        ...
        >
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              type="text"
              placeholder="ຄົ້ນຫາຊ່ອງທາງຮັບແຈ້ງ..."
              value={search}
                  ...
                  {headerGroup.headers.map((header) => {
                    const widthClass =
                      header.id === "index" ? "w-[10%] min-w-[70px]" :
                        header.id === "name" ? "w-[30%] min-w-[150px]" :
                          header.id === "description" ? "w-[45%] min-w-[200px]" :
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນຊ່ອງທາງຮັບແຈ້ງ</p>
                      </div>
                    </div>
                  </td>
                </tr>
                    ...
                    {row.getVisibleCells().map((cell) => {
                      const widthClass =
                        cell.column.id === "index" ? "w-[10%] min-w-[70px]" :
                          cell.column.id === "name" ? "w-[30%] min-w-[150px]" :
                            cell.column.id === "description" ? "w-[45%] min-w-[200px]" :
      ...
      {/* MODALS */}
      <AddSourceTypeModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />
