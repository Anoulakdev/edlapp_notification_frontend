...
// Modals
import { AddTurnoffModal } from "./AddTurnoffModal";
import { EditTurnoffModal } from "./EditTurnoffModal";
import { DeleteTurnoffModal } from "./DeleteTurnoffModal";
import { ViewTurnoffModal } from "./ViewTurnoffModal";

const ROWS_PER_PAGE = 10;

export function TurnoffManagement() {
  const router = useRouter();
  const [docs, setDocs] = useState<TurnoffDoc[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<TurnoffDoc | null>(null);
  ...
  const fetchDocs = useCallback(async (searchVal = "", start = "", end = "", provId = "", distId = "", myDocsOnly = false) => {
    try {
      ...
      console.error("Failed to load outage documents:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນເອກະສານແຈ້ງການມອດໄຟໄດ້");
  ...
  const openEdit = (doc: TurnoffDoc) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openAssignVillages = (doc: TurnoffDoc) => {
    const encrypted = encryptId(doc.id);
    router.push(`/turnoffassign?id=${encrypted}`);
  };

  const openDelete = (doc: TurnoffDoc) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/turnoffdocs/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບເອກະສານແຈ້ງການມອດໄຟສຳເລັດ");

    } catch (err) {
      console.error("Failed to delete outage document:", err);
      toast.error("ເກີດຂໍ້ຜິດພາດໃນການລົບເອກະສານ");
    }
  };

  // Columns definition
  const columns = useMemo<ColumnDef<TurnoffDoc>[]>(
    () => [
      {
        accessorKey: "id",
        header: "ເລກທີ",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          const formattedStart = moment(doc.startDate).format("DD/MM/YYYY");
          const formattedEnd = moment(doc.endDate).format("DD/MM/YYYY");
          const isSame = formattedStart === formattedEnd;
          return (
            <span className="text-xs font-medium text-slate-600 dark:text-slate-350 flex items-center gap-1.5 whitespace-nowrap">
              ...
              {isSame ? formattedStart : `${formattedStart} - ${formattedEnd}`}
            </span>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const doc = row.original;
          return (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-semibold bg-brand/10 text-brand dark:bg-brand/20 whitespace-nowrap">
        ...
        {showFilters && (
          ...
          >
            <div className="w-full sm:w-48 flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase">ວັນທີເລີ່ມຕົ້ນ</label>
              <input
                type="date"
                value={startDate}
            ...
            <div className="w-full sm:w-48 flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase">ວັນທີສິ້ນສຸດ</label>
              <input
                type="date"
                value={endDate}
