...
import { createMeterStatusSchema } from "@/schemas/meterstatus";
import { toast } from "react-toastify";

interface AddMeterStatusModalProps {
  open: boolean;
  onClose: () => void;
  onRefresh: () => void;
}

export function AddMeterStatusModal({ open, onClose, onRefresh }: AddMeterStatusModalProps) {
  ...
  const handleSubmit = async () => {
    const result = createMeterStatusSchema.safeParse({
      edlapp,
      callcenter,
    });
    ...
    try {
      await axiosInstance.post("/meterstatues", {
        edlapp,
        callcenter,
      });

      toast.success("ເພີ່ມຂໍ້ມູນສະຖານະຂໍໝໍ້ນັບໄຟສຳເລັດ");
      onRefresh();
      onClose();
    } catch (err: any) {
      console.error("Failed to add meter status:", err);
  ...
  return (
    <Modal open={open} onClose={onClose} title="ເພີ່ມຂໍ້ມູນສະຖານະຂໍໝໍ້ນັບໄຟ" size="md">
