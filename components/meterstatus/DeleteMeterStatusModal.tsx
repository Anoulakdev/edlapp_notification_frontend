...
interface DeleteMeterStatusModalProps {
  open: boolean;
  onClose: () => void;
...
export function DeleteMeterStatusModal({ open, onClose, onDelete, selectedDoc }: DeleteMeterStatusModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="ລົບຂໍ້ມູນສະຖານະຂໍໝໍ້ນັບໄຟ" size="sm">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="text-sm font-semibold truncate" style={{ color: "rgb(var(--text-primary))" }}>
              {selectedDoc?.edlapp} ({selectedDoc?.callcenter})
            </div>
          </div>
        </div>
