...
import { MessageAuto } from "@/schemas/messageauto";

// Modals
import { AddMessageAutoModal } from "./AddMessageAutoModal";
import { EditMessageAutoModal } from "./EditMessageAutoModal";
import { DeleteMessageAutoModal } from "./DeleteMessageAutoModal";
...
export function MessageAutoManagement() {
  const router = useRouter();
  const [docs, setDocs] = useState<MessageAuto[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<MessageAuto | null>(null);

  // Authenticate user & role check (Roles 2, 4 in backend controller)
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const res = await axiosInstance.get("/auth/me");
        if ([2, 4].includes(res.data?.roleId)) {
          setAuthorized(true);
        } else {
  ...
  // Fetch Message Autos & Topics list
  const fetchDocs = useCallback(async () => {
    try {
      setLoading(true);
      const [msgRes, topicsRes] = await Promise.all([
        axiosInstance.get("/messageautos"),
      ...
      const topicData = Array.isArray(topicsRes.data)
        ? topicsRes.data
        : (topicsRes.data?.data || []);
      setTopics(topicData);
    } catch (err) {
      console.error("Failed to load message autos:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນຂໍ້ຄວາມອັດໂນມັດໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (doc: MessageAuto) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: MessageAuto) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/messageautos/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນຂໍ້ຄວາມອັດໂນມັດສຳເລັດ");
    } catch (err) {
      console.error("Failed to delete message auto:", err);
      toast.error("ເກີດຂໍ້ຜິດພາດໃນການລົບຂໍ້ມູນ");
    }
  };

  // Columns definition
  const columns = useMemo<ColumnDef<MessageAuto>[]>(
    () => [
      {
        id: "index",
        header: "ລຳດັບ",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/50 whitespace-normal">
              {doc.topic?.name || "-"}
            </span>
          );
        },
      ...
      {
        accessorKey: "messageTopic",
        header: "ຂໍ້ຄວາມອັດໂນມັດ",
        cell: ({ row }) => {
          const doc = row.original;
          return (
  ...
  // Client-side filtering (Search & Topic filter)
  const filteredDocs = useMemo(() => {
    return docs.filter((doc) => {
      // Topic filter
      if (selectedTopicId && String(doc.topicId) !== selectedTopicId) {
        return false;
      }
      // Search term filter
      if (search.trim()) {
        const s = search.toLowerCase().trim();
        const matchMsg = doc.messageTopic.toLowerCase().includes(s);
        const matchTopic = doc.topic?.name.toLowerCase().includes(s);
        return matchMsg || matchTopic;
      }
      return true;
    });
  }, [docs, search, selectedTopicId]);
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} style={{ background: "rgb(var(--bg))" }}>
                  {headerGroup.headers.map((header) => {
                    const widthClass =
                      header.id === "index" ? "w-[8%] min-w-[60px]" :
                        header.id === "topic" ? "w-[25%] min-w-[150px]" :
                          header.id === "messageTopic" ? "w-[42%] min-w-[220px]" :
                            header.id === "createdAt" ? "w-[15%] min-w-[130px]" :
                              header.id === "actions" ? "w-[10%] min-w-[90px]" : "";
                    return (
                      <th
                        key={header.id}
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນຂໍ້ຄວາມອັດໂນມັດ</p>
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => {
                      const widthClass =
                        cell.column.id === "index" ? "w-[8%] min-w-[60px]" :
                          cell.column.id === "topic" ? "w-[25%] min-w-[150px]" :
                            cell.column.id === "messageTopic" ? "w-[42%] min-w-[220px]" :
                              cell.column.id === "createdAt" ? "w-[15%] min-w-[130px]" :
                                cell.column.id === "actions" ? "w-[10%] min-w-[90px]" : "";
                      return (
      ...
      {/* MODALS */}
      <AddMessageAutoModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />

      <EditMessageAutoModal
        open={editOpen}
