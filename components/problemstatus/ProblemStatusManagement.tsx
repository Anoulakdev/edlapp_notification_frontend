...
// Modals
import { AddProblemStatusModal } from "./AddProblemStatusModal";
import { EditProblemStatusModal } from "./EditProblemStatusModal";
import { DeleteProblemStatusModal } from "./DeleteProblemStatusModal";

const ROWS_PER_PAGE = 10;

export function ProblemStatusManagement() {
  const router = useRouter();
  const [docs, setDocs] = useState<ProblemStatus[]>([]);
  ...
  const [selectedDoc, setSelectedDoc] = useState<ProblemStatus | null>(null);
  ...
  const fetchDocs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/problemstatus");
      const data = Array.isArray(res.data) ? res.data : (res.data.data || []);
      setDocs(data);
    } catch (err) {
      console.error("Failed to load problem statuses:", err);
      toast.error("ບໍ່ສາມາດໂຫຼດຂໍ້ມູນສະຖານະບັນຫາໄດ້");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (doc: ProblemStatus) => {
    setSelectedDoc(doc);
    setEditOpen(true);
  };

  const openDelete = (doc: ProblemStatus) => {
    setSelectedDoc(doc);
    setDeleteOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/problemstatus/${selectedDoc.id}`);
      setDocs((prev) => prev.filter((d) => d.id !== selectedDoc.id));
      setDeleteOpen(false);
      toast.success("ລົບຂໍ້ມູນສະຖານະບັນຫາສຳເລັດ");
    } catch (err) {
  ...
  // Columns definition
  const columns = useMemo<ColumnDef<ProblemStatus>[]>(
    () => [
      {
        id: "index",
        header: "ລຳດັບ",
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-6 py-8" style={{ fontFamily: "'Noto Sans Lao', sans-serif" }}>
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold">ບໍ່ພົບຂໍ້ມູນສະຖານະບັນຫາ</p>
                      </div>
                    </div>
                  </td>
                </tr>
      ...
      {/* MODALS */}
      <AddProblemStatusModal open={addOpen} onClose={() => setAddOpen(false)} onRefresh={fetchDocs} />

      <EditProblemStatusModal
        open={editOpen}
